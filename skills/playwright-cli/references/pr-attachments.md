...
`gh` 2.99+ uploads local images and videos with the repeatable `--attach` flag on `gh pr create`, `gh pr comment`, `gh pr edit`, `gh issue create`, `gh issue comment` and `gh issue edit`. PNG, JPEG, GIF, WebP, SVG, MP4, MOV and WebM are accepted, so `npx @playwright/cli screenshot` and `video-start` output can be attached as is.
...
npx @playwright/cli open http://localhost:3000/settings
npx @playwright/cli screenshot --filename=settings-after.png
npx @playwright/cli video-start settings-flow.webm
npx @playwright/cli click e5
npx @playwright/cli fill e7 "New name" --submit
