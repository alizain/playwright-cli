...
Once instructions are printed, use `npx @playwright/cli` to explore the page. Debugging instructions include a browser name that should be used in `npx @playwright/cli` to attach to the page under test.
...
npx @playwright/cli --session=test open --attach=test-worker-abcdef
npx @playwright/cli --session=test snapshot
npx @playwright/cli --session=test click e14
```
